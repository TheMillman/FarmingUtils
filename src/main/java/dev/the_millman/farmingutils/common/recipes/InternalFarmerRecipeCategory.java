...
public class InternalFarmerRecipeCategory implements IRecipeCategory<InternalFarmerRecipe>{
	...
	public void setRecipe(IRecipeLayoutBuilder builder, InternalFarmerRecipe recipe, IFocusGroup focuses) {
		builder.addSlot(RecipeIngredientRole.INPUT, 27-3, 18-6).addIngredients(recipe.getIngredients().get(0));
		builder.addSlot(RecipeIngredientRole.INPUT, 27-3, 54-6).addIngredients(recipe.getIngredients().get(1));
		...
		builder.addSlot(RecipeIngredientRole.OUTPUT, 80-3, 36-6).addItemStack(recipe.getResultItem());
	}
